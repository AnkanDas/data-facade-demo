...
class ExecutionHandler(BaseExecutionHandler):
    def execute(self, df1: pd.DataFrame, df2: pd.DataFrame, df1_key, df2_key, df2_new_col_add, product_param, join_type='left'):
        try:
            df = pd.merge(df1, df2[[df2_key, df2_new_col_add]], how = join_type, left_on=df1_key, right_on=df2_key).drop(df2_key,axis=1)
