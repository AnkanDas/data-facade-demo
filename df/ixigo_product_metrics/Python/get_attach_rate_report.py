...
class ExecutionHandler(BaseExecutionHandler):
    def execute(self, data, group_column, product_param, to_date_str, from_date_str = ''):
        ...
        try:
            ...
            report = data.groupby(group_column).agg({'bookingId':'count'}).rename(columns = {'bookingId':'Total_Bookings'})
            report[['Insured_Bookings','Avg_Premium']] = data[data[product_param]==1].groupby(group_column).agg({'bookingId':'count', 'total_premium':'sum'})
